...
class nashAssigner:
	...
	def __init__(self,N,M,R,tau,dist=None,arrivalTime={},decisionTime=0): #initialize 
		...
		# ~ print('\nN veh:',self.N,'Veh count:',env.traci.vehicle.getIDCount())
	...
	def getAssignments(self): 
		...
		while v:  #while queue is not empty 
            ...
            # ??? Quentin
			# ~ print(self.N,env.traci.vehicle.getIDCount())
            # ???
			...
			try:
				...
				print("systematicSim::92 Catching Index Error: v1=",v1)
				v1 = self.N-1
            # ???
		...
		for target in self.arrivalTime:#for each target 
			...
			for i,vehicle in enumerate(vehicles):
				...
				try:
					...
					print("systematicSim::128 Catching Index Error: vehicle[0]=",vehicle[0])
					vehicles[i] = list(vehicles[i])
					vehicles[i][0] = self.N - 1
					vehicles[i] = tuple(vehicles[i])
					vehicle = vehicles[i]
                # ???
